...
wss.on("connection", (ws, req) => {
  ...
  console.log(`[+] ${isLaptop ? "Laptop" : "Browser"} connected (${clients.size} total)`);

  // Broadcast status to all browser clients
  broadcast();

  ws.on("close", () => {
    clients.forEach((c) => { if (c.ws === ws) clients.delete(c); });
    console.log(`[-] ${isLaptop ? "Laptop" : "Browser"} disconnected (${clients.size} total)`);
    broadcast();
  });
